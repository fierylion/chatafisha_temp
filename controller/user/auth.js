        ...
        if (!err && feedback.length > 0) {
            ...
            bcpt.compare(req.body.password, feedback[0].password).then(validit => {
                if (validit == true) {
                    ...
                    // check user role
                    feedback[0].role == 1 ?
                        user_data["role_id"] = "Admin" : feedback[0].role == 2 ? user_data["role_id"] = "Picker" : feedback[0].role == 3 ? user_data["role_id"] = "Student" : user_data["role_id"]==4? user_data["role_id"]="Validator":user_data["role_id"] = "Unknown"
